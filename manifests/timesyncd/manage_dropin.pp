...
# @param timesyncd_entry key value pairs for the [Time] section of the dropin file
define systemd::timesyncd::manage_dropin (
  Hash                            $timesyncd_entry,
  ...
  Optional[Boolean]               $notify_timesyncd        = undef,
) {
  systemd::timesyncd::dropin_file { $name:
    ensure                  => $ensure,
    filename                => $filename,
    path                    => $path,
    ...
    mode                    => $mode,
    show_diff               => $show_diff,
    notify_timesyncd        => $notify_timesyncd,
    content                 => epp('systemd/config_dropin.epp', {
      'section'  => 'Time',
      'settings' => $timesyncd_entry,
      'comments' => $comments,
    }),
  }
}
