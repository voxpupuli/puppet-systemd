...
# @param logind_entry key value pairs for the [Login] section of the dropin file
define systemd::logind::manage_dropin (
  Systemd::LogindSettings        $logind_entry,
  ...
  Optional[Boolean]              $notify_logind           = undef,
) {
  systemd::logind::dropin_file { $name:
    ensure                  => $ensure,
    filename                => $filename,
    path                    => $path,
    ...
    mode                    => $mode,
    show_diff               => $show_diff,
    notify_logind           => $notify_logind,
    content                 => epp('systemd/config_dropin.epp', {
      'section'  => 'Login',
      'settings' => $logind_entry,
      'comments' => $comments,
    }),
  }
}
