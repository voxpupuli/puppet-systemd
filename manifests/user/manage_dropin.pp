...
# @param user_entry key value pairs for the [Manager] section of the dropin file
define systemd::user::manage_dropin (
  Systemd::ServiceManagerSettings $user_entry,
  ...
  Optional[Boolean]               $notify_user             = undef,
) {
  systemd::user::dropin_file { $name:
    ensure                  => $ensure,
    filename                => $filename,
    path                    => $path,
