...
describe 'systemd::sleep::dropin_file' do
  context 'supported operating systems' do
    on_supported_os.each do |os, facts|
      context "on #{os}" do
        ...
        it {
          is_expected.to contain_file('/etc/systemd/sleep.conf.d')
        ...
        context 'with notify_sleep set to false' do
          let(:params) { { notify_sleep: false } }
        ...
        context 'with purge_dropin_dirs set to true' do
          let(:pre_condition) { 'class { "systemd": manage_sleep => true, sleep_purge_dropin_dirs => true }' }
        ...
        context 'with systemd::manage_sleep set to false' do
          let(:pre_condition) { 'class { "systemd": manage_sleep => false }' }

          it { is_expected.to compile.and_raise_error(%r{systemd::sleep::dropin_file is disabled because systemd::manage_sleep is set to false}) }
        end
      end
    end
  end
