...
describe 'systemd::system::dropin_file' do
  context 'supported operating systems' do
    on_supported_os.each do |os, facts|
      context "on #{os}" do
        ...
        it {
          is_expected.to contain_file('/etc/systemd/system.conf.d')
        ...
        context 'with notify_system set to false' do
          let(:params) { { notify_system: false } }
        ...
        context 'with purge_dropin_dirs set to true' do
          let(:pre_condition) do
            [
              'systemd::daemon_reexec { "system.conf": }',
              'class { "systemd": system_purge_dropin_dirs => true }',
            ].join("\n")
          end
