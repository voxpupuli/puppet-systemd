    ...
    on_supported_os.each do |os, facts|
      context "on #{os}" do
        ...
        context('with timer_content and service_content') do
          ...
          it { is_expected.to contain_systemd__unit_file('foobar.timer').with(
           :content => "[Timer]\nOnCalendar=weekly",
          ) }

          it { is_expected.to contain_systemd__unit_file('foobar.service').with(
           :content => "[Service]\nExecStart=/bin/touch /tmp/foobar",
          ) }
        end
        ...
        context 'with a bad timer name' do
          ...
          it {
            ...
            }.to raise_error(/expects a match for/)
          }
        end
      end
    end
